...
Some relations accept additional parameters (for example, the regional-statistics relations can be narrowed to a specific reference year). Send them as a JSON object in the **request body**, with `Content-Type: application/json`:
...
Fetch the **Arbeitslosenquote** (unemployment rate) regional statistic:
